/**
 * Servidor WebSocket + HTTP publish para producción (Railway, Render, VPS).
 * No corre en Vercel. Vercel envía eventos con POST a WS_PUBLISH_URL.
 *
 * Uso: node scripts/servidor-ws-produccion.mjs
 * Variables: PORT, WS_PUBLISH_SECRET
 */
...
const servidor = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && req.url === "/health") {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ ok: true, clientes: clientes.size }));
    return;
  }
...
servidor.listen(PORT, "0.0.0.0", () => {
  console.log(`[PawPatrol WS] Puerto ${PORT}`);
  console.log(`[PawPatrol WS] WebSocket: ws(s)://<host>:${PORT}`);
  console.log(`[PawPatrol WS] Publicar: POST /publish (Bearer WS_PUBLISH_SECRET)`);
  console.log(`[PawPatrol WS] Salud: GET /health`);
});
