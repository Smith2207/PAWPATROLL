/**
 * Indexa TODAS las fotos (hasta CLIP_MAX_FOTOS_INDEX) de mascotas perdidas.
 * Modelo en disco: .cache/transformers/ (Xenova/clip-vit-base-patch32)
 */
...
console.log(`Modelo: ${MODELO_CLIP}`);
console.log(`Cache:  ${carpetaCacheModelo}`);
console.log(`Mascotas: ${porMascota.size} · Fotos a indexar: ${totalFotos}`);
...
for (const [, fotos] of porMascota) {
  ...
  for (const foto of fotos) {
    ...
    try {
      ...
      await sql`
        INSERT INTO mascota_embedding (id, mascota_id, foto_id, embedding, modelo, updated_at)
        VALUES (${id}, ${mascotaId}, ${foto.foto_id}, ${json}, ${modelo}, now())
